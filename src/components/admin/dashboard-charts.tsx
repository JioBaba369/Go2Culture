    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Total booking volume over the last few months.</CardDescription>
            </CardHeader>
            <CardContent>
                <ChartContainer config={chartConfig} className="h-64">
                    <BarChart data={data} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
...
export function EarningsChart({ bookings }: { bookings: Booking[] }) {
    const data = useMemo(() => {
        ...
        const earnings = confirmedBookings.reduce((acc, booking) => {
            ...
            const month = format(date, 'MMM yyyy');
            // Assuming 15% platform commission for revenue
            acc[month] = (acc[month] || 0) + (booking.totalPrice * 0.15);
            return acc;
        }, {} as Record<string, number>);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Estimated monthly revenue from confirmed bookings (15% commission).</CardDescription>
            </CardHeader>
            <CardContent>
                <ChartContainer config={chartConfig} className="h-64">
                    <BarChart data={data} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
