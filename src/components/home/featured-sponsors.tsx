...
export function FeaturedSponsorsSection() {
    ...
    const sponsorsQuery = useMemoFirebase(
        () => firestore ? query(collection(firestore, 'sponsors')) : null,
        [firestore]
    );
    const { data: allSponsors, isLoading } = useCollection<Sponsor>(sponsorsQuery);

    const activeSponsors = useMemo(() => {
        if (!allSponsors) return null;
        return allSponsors.filter(s => s.isActive);
    }, [allSponsors]);


    if (isLoading || !activeSponsors || activeSponsors.length === 0) {
